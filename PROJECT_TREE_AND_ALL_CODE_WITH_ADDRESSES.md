# MockMate-pro: Tree and Full Code With File Addresses

- Generated on: 2026-04-19
- Workspace root: `/home/nikhil-pal/Desktop/MockMate-pro`
- File addresses are shown as absolute and relative paths for each file section.
- Excluded: `.git`, `node_modules`, and build output folders.

## Full Folder/File Tree

```text
.
backend
backend/config
backend/config/db.js
backend/controllers
backend/controllers/ai.cotroller.js
backend/controllers/auth.controller.js
backend/controllers/company.controller.js
backend/controllers/payment.controller.js
backend/controllers/plan.controller.js
backend/controllers/session.controller.js
backend/.env
backend/.gitignore
backend/middleware
backend/middleware/auth.middleware.js
backend/middleware/error.middleware.js
backend/models
backend/models/Company.js
backend/models/InterviewConfig.js
backend/models/Otp.js
backend/models/Plan.js
backend/models/Role.js
backend/models/Session.js
backend/models/User.js
backend/package.json
backend/package-lock.json
backend/routes
backend/routes/auth.routes.js
backend/routes/company.routes.js
backend/routes/payment.routes.js
backend/routes/plan.routes.js
backend/routes/session.routes.js
backend/server.js
backend/services
backend/services/ai.service.js
backend/test-ai.js
backend/utils
backend/utils/constants.js
backend/utils/sendEmail.js
client
client/.env
client/eslint.config.js
client/.gitignore
client/index.html
client/package.json
client/package-lock.json
client/postcss.config.js
client/public
client/public/favicon.svg
client/public/icons.svg
client/README.md
client/src
client/src/App.css
client/src/App.jsx
client/src/assets
client/src/assets/hero.png
client/src/assets/react.svg
client/src/assets/vite.svg
client/src/components
client/src/components/Loader.jsx
client/src/components/Navbar.jsx
client/src/components/ProtectedRoute.jsx
client/src/context
client/src/context/AuthContext.jsx
client/src/index.css
client/src/main.jsx
client/src/pages
client/src/pages/CompanyPrep.jsx
client/src/pages/Dashboard.jsx
client/src/pages/Feedback.jsx
client/src/pages/ForgotPassword.jsx
client/src/pages/History.jsx
client/src/pages/Login.jsx
client/src/pages/PracticeSelect.jsx
client/src/pages/Premium.jsx
client/src/pages/Profile.jsx
client/src/pages/Register.jsx
client/src/pages/RoleSelect.jsx
client/src/pages/Session.jsx
client/src/pages/VerifyOtp.jsx
client/src/services
client/src/services/api.js
client/tailwind.config.js
client/vite.config.js
PROJECT_TREE_AND_ALL_CODE_WITH_ADDRESSES.md
README_EXTERNAL_MODEL.md
README_EXTERNAL_MODEL_SEPARATE.md
```

## Code of All Files (With Addressing)

### `backend/config/db.js`

