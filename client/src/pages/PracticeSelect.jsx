...
const PracticeSelect = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([sessionAPI.getRoles(), sessionAPI.getConfig()])
      ...
      .catch(() => toast.error('Failed to load practice config'))
          ...
          <div className="grid grid-cols-3 gap-3">
            {diffs.map((d) => (
              <button key={d.value} onClick={() => setDiff(d.value)}
                className={`p-3.5 rounded-xl border-2 text-left transition-all ${diff === d.value ? (DIFF_STYLES[d.value] || 'border-teal-400 bg-teal-50 text-teal-700') : 'border-gray-200 bg-white text-gray-700 hover:border-gray-300'}`}>
